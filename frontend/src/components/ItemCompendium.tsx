        ...
        <div className="modal-overlay" onClick={() => setSelectedItem(null)}>
          <div className="bestiary-detail-modal" onClick={(e) => e.stopPropagation()}>
            <div className="bestiary-detail-header">
              <div>
                ...
                <p className="bestiary-subtitle">
                  ...
                  {selectedItem.attunement ? " · Attunement requis" : ""}
                </p>
              </div>
            ...
            <div className="bestiary-stats-grid">
              ...
              {selectedItem.damage && (
                <div>
                  <strong>Damage</strong> {selectedItem.damage}
                </div>
              )}
              {selectedItem.ac !== null && selectedItem.ac !== undefined && (
                <div>
                  <strong>AC</strong> {selectedItem.ac}
                  {selectedItem.armor_type ? ` (${selectedItem.armor_type})` : ""}
                </div>
              )}
            </div>
            {selectedItem.properties.length > 0 && (
              <div className="bestiary-detail-section">
                <strong>Properties</strong> {selectedItem.properties.join(", ")}
              </div>
            )}
            <div className="bestiary-detail-section">
            ...
            <div className="bestiary-detail-footer">
              <em>Source: {selectedItem.source}</em>
            </div>
          </div>
        </div>
      )}
